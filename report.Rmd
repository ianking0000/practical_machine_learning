...
This report shows the development of a machine learning algrithm that can predict the manner in which people did exercise. The manner is stored as *classe* in the data set.
...
1. The code below is for a regular decision three using "rpart" method:  
...
As we can see the out-of-sample accuracy is only `r ainfo3`, which is farily low. This is reasonalbe since without pruning/bootstrp the decision tree tends to have overfitting prolbmes so its out-of-sample error is large.   

2. The code below is for a regular decision three using "rpart" method. It is known that ramdonForest has the bootstrap/crossvalidation within the algorithm itself so it should have a better performance that simply decision tree.  
...
tinfo  
```  
As we can see the overall accuracy is `r ainfo`, **which means we could expect the out-of-sample error is only `r 1-ainfo`, which is farily low.**  
...
The  randomForest algorithm with importance=TRUE could give a relatively accurate model to predict "classe" varaibles. The generated model has been applied to the really test set and gave 100% accuracy.  